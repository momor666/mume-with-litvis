    ...
    for (const path of dependentSchemaPaths) {
      ...
      try {
        const fileInMemory = _.find(
          filesInMemory,
          (f: LitvisDocument) => f.path === resolvedPath,
        );
        file = fileInMemory
          ? vfile(fileInMemory)
      ...
      let rawData;
      try {
        rawData = yaml.parse(file.contents);
      } catch (e) {
        try {
          file.fail(
...
const resolveNarrativeSchemaPath = async (
  path,
  file: LitvisDocument | NarrativeSchema,
): Promise<string> => {
...
): Array<LitvisDocument | NarrativeSchema> => {
  const result = [];
  let currentFile: LitvisDocument | NarrativeSchema = narrativeSchema;
  while (currentFile) {
